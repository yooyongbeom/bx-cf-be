...
기존 클라이언트 계약을 최소한으로 변경하기 위해 수정 작업자 필드명은 `createdBy`를 유지한다. `menuId`는 PathVariable로만 전달하며 `menuCd`는 수정 대상에서 제외한다.
...
- 등록 SQL은 `ApiRequest<MenuCreateReqDto>`의 `data` 필드를 사용한다.
- 수정 SQL은 별도 `menuId` Mapper 파라미터와 `ApiRequest<MenuUpdateReqDto>`를 사용한다. 수정 DTO에 식별자를 주입하지 않는다.
